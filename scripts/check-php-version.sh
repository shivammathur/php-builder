...
get_stable_release_tag() {
  ...
  if [ "$source" = "--web-php" ]; then
    ...
    release_tag="${release_tag:-$(curl -sL https://www.php.net/releases | grep -Po "<h2>\K${php_version//./\\.}\.[0-9]+" | head -n1 | sed 's/^/php-/')}"
...
if [[ "$2" = *build-all* ]]; then
  ...
  for php_version in "${php_versions_to_check[@]}"; do
    ...
    if [ -z "$new_version" ] || {
      ...
      [ "$(printf '%s\n' "$existing_version" "$new_version" | sort -V | tail -n 1)" = "$new_version" ]; }; then
