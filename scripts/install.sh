...
local_deps() {
  ...
  [[ "$version" =~ 5.6|7.[0-2] && "$VERSION_ID" != "26.04" ]] && libpcre_dev=libpcre3-dev || libpcre_dev=libpcre2-dev
  add_ppa
...
github_deps() {
  ...
  if [ "$VERSION_ID" = "22.04" ]; then
    ...
    resolute_libs=('unixodbc-dev' 'libmagickcore-dev' 'libxmlrpc-epi-dev')
    [[ "$version" =~ 5.6|7.[0-2] ]] && resolute_libs+=('libpcre2-dev')
    install_packages "${resolute_libs[@]}"
  fi
